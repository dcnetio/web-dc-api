...
export class KeyManager {
  ...
  static async deriveKeyForChain(
    ...
    return await this.deriveKey(mnemonic, DERIVATION_PATHS[chain], password);
  }
  // 安全清除数据
...
export async function ed25519PublicKeyToCryptoKey(
  ...
  if (typeof publicKey === "string") {
    ...
    if (publicKey.startsWith("-----BEGIN")) {
      ...
      const pemContents = publicKey
        ...
        .replace(pemFooter, "")
        .replace(/\s+/g, "");
      keyData = Uint8Array.from(atob(pemContents), (c) =>
        c.charCodeAt(0)
      ) as any;
    } else {
  ...
  const detectedFormat =
    format === "spki" || publicKey instanceof Uint8Array ? format : "spki";
...
export async function ed25519PrivateKeyToCryptoKey(
  ...
  if (typeof privateKey === "string") {
    ...
    if (privateKey.startsWith("-----BEGIN")) {
      ...
      keyData = Uint8Array.from(atob(privateKey), (c) =>
        c.charCodeAt(0)
      ) as any;
    }
  } else {
  ...
  // 2. 检测密钥格式
  const detectedFormat =
    format === "pkcs8" || privateKey instanceof Uint8Array ? format : "pkcs8";
