...
export class Encryption {
  ...
  // 修改转换函数中的模逆计算
  private static ed25519PublicKeyToCurve25519(
    ...
    try {
      // 使用新的安全模逆计算
      const denominatorInv = this.safeModInverse(
        denominator,
        this.CURVE25519_P
      );
  ...
  static async encrypt(
    ...
    try {
      if (recipientPublicKey.length !== this.PUBLIC_KEY_LENGTH) {
        throw new Error(
          `Invalid public key length: ${recipientPublicKey.length}`
        );
      }
      ...
      const result = new Uint8Array(
        this.NONCE_LENGTH + this.PUBLIC_KEY_LENGTH + encrypted.length
      );
  ...
  static async encryptWithDebug(
    ...
    try {
      ...
      console.log(
        `  * Nonce (${this.NONCE_LENGTH} bytes):`,
        this.toHex(result.slice(0, this.NONCE_LENGTH))
      );
      console.log(
        `  * Ephemeral Public Key (${this.PUBLIC_KEY_LENGTH} bytes):`,
        this.toHex(
          result.slice(
            this.NONCE_LENGTH,
  ...
  static async decrypt(
    ...
    try {
      ...
      // 验证输入数据长度
      const minLength =
        this.NONCE_LENGTH + this.PUBLIC_KEY_LENGTH + this.MAC_LENGTH;
      ...
      // 转换私钥（现在是异步的）
      const curve25519PrivateKey = await this.ed25519PrivateKeyToCurve25519(
        recipientPrivateKey
      );
      ...
      if (debug) {
        console.log("\n--- Key Conversion ---");
        console.log(
          "Original Ed25519 Private Key:",
          this.toHex(recipientPrivateKey)
        );
        console.log(
          "Converted Curve25519 Private Key:",
          this.toHex(curve25519PrivateKey)
        );
      }
