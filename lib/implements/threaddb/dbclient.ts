...
export class DBClient {
  ...
  async requestThreadID(peerAddr?: TMultiaddr): Promise<string> {
    try {
      ...
      const grpcClient = new DBGrpcClient(this.client, this.net, peerAddr);
  ...
  async createThread(tid: string, opts: NewThreadOptions): Promise<string> {
    try {
      ...
      const sPubkey = await extractPublicKeyFromPeerId(
        this.client.p2pNode.peerId
      );
  ...
  async pushLogToPeer(
    ...
    try {
      ...
      const grpcClient = new DBGrpcClient(this.client, this.net);
      await grpcClient.pushLogToPeer(tid, lid, this.logstore);
      await this.dc.createTransferStream(
        this.client.p2pNode,
        this.client.blockstore,
        this.client.peerAddr,
  ...
  async exchangeEdges(threadIds: ThreadID[]): Promise<void> {
    try {
      ...
      console.debug(
        `exchanging edges of ${threadIds.length} threads with peer...`
      );
      ...
      try {
        ...
        if (err.code === "UNIMPLEMENTED") {
          console.debug(
            `Peer doesn't support edge exchange, falling back to direct record pulling`
          );
          for (const tid of threadIds) {
            await this.scheduleUpdateRecords(tid);
            console.debug(`Record update for thread ${tid} scheduled`);
  ...
  private async pushLogsHeadToPeer(tid: ThreadID): Promise<void> {
    ...
    for (const log of logs) {
      ...
      if (head.length > 0) {
        //取最新的head
        headToPush = head.reduce((prev, current) =>
          prev.counter > current.counter ? prev : current
        );
        if (headToPush.id && headToPush.counter > 0) {
          const rec = await GetRecord(
            this.net.getDagService(),
            headToPush.id,
            sk,
            true
          );
          if (rec) {
            await this.pushRecordToPeer(tid, log.id, rec, headToPush.counter);
          }
        }
      }
    }
   ...
   */
  private async localEdges(
    tid: ThreadID
  ): Promise<{ addrEdge: bigint; headsEdge: bigint }> {
    // 使用默认值初始化头部边缘值
  ...
  async getLogs(tid: ThreadID): Promise<IThreadLogInfo[]> {
    try {
      ...
      console.warn(
        `Get logs from ${this.client.peerAddr} failed: ${
          err instanceof Error ? err.message : String(err)
        }`
      );
      throw err;
  ...
  async getRecordsFromPeer(
    ...
    const reply = await grpcClient.getRecordsFromPeer(
      req,
      SymmetricKey.fromSymKey(serviceKey)
    );
