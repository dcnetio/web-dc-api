...
private async isAppAccountExists(pubkeyStr: string): Promise<boolean> {
 ...
 async setUserDefaultDB(threadId: string, rk: string,sk: string,remark: string,vaccount?: string): Promise<void>{
  ...
  const dbinfocrypto = await this.context.publicKey.encrypt(new TextEncoder().encode(dbinfo));
  const blockHeight = await this.chainUtil?.getBlockHeight();
  if (blockHeight === undefined) {
    throw new AccountError("Failed to get blockchain height");
  }
   if(this.connectedDc.client.token == ""){
        const token = await this.connectedDc.client.GetToken(this.context.publicKey.string(), async (payload: Uint8Array): Promise<Uint8Array> => {
