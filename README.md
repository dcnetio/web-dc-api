...
- 如果一个生成的代码文件里面包含了对keyValue DB的创建操作, 必须在文件头部标注使用到的主题名称,方便数据库静态构建.
  格式如下: `/**keyValueDB: keyvalue_app_config_pub */` 每个主题一行,如果有多个主题,请逐行添加
- 所有涉及到keyValue DB的操作的代码文件都应该放在services目录下
- 无论任何情况下，都必须创建/src/scripts/initdata.json文件,用于存放需要初始化的数据
- 必须为每个keyValue DB的公共主题创建一个初始化脚本,放在scripts目录下,脚本文件名格式: `init_keyvalue_{主题名称}.ts`,例如: `init_keyvalue_app_config_pub.ts`,初始化脚本内容必须包含创建主题、配置权限、以及初始化数据的代码,方便后续一键初始化数据库,初始化的数据来源必须放在scripts目录下的initdata.json文件中
- scripts目录下的initdata.json文件中,每次代码修改过程中涉及初始化数据调整,都需要进行对应调整;文件内容按主题分类,每个主题的初始化数据,按key进行分类,可以有多个key,每个key的数据内容为JSON,包括value,和indexs(indexs可以为空),格式如下:
```json
{
  "keyvalue_app_config_pub":  // 主题名称1
  key1:{// key名称
     value: { // 数据内容
    ...
    indexs: [{key:"type",type:"string",value:"config"}] // 索引配置
  },
  key2:{// key名称
     value: { // 数据内容
  ...
  "keyvalue_announcements_pub": { // 主题名称2,无索引情况
    "announcement_1": {
      value: {
    ...
    "announcement_2": {
      value: {
        ...
        "content": "Our service will be down for maintenance on Saturday from 1 AM to 3 AM."
      }
    }
  }
}
