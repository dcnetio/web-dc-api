...
export class DBGrpcClient {
  ...
  async createThread(tid: string, opts: NewThreadOptions): Promise<ThreadInfo> {
    try {
      ...
      message.keys = await this.getThreadKeys(sPubkey, {
  ...
  async addLogToThread(
    ...
    try {
      ...
      message.signature = opts.signature;
      const messageBytes =
        dcnet_proto.pb.AddLogToThreadRequest.encode(message).finish();
  ...
  async threadInfoFromProto(
    ...
    const logs: IThreadLogInfo[] = reply.logs
      ? await Promise.all(
          reply.logs.map(async (lg) => {
            ...
            const addrs =
              lg.addrs?.map((addr) => MultiaddrConverter.fromBytes(addr)) || [];
    ...
    const threadInfo = new ThreadInfo(threadID, logs, addrs);
    return threadInfo;
  }
  ...
  async getThreadKeys(
    ...
    try {
      ...
      const keys = new dcnet_proto.pb.Keys({
        threadKeyEncrpt: threadKeyEncrypt,
        logKeyEncrpt: logKeyEncrpt,
      });
      return keys;
    } catch (err) {
